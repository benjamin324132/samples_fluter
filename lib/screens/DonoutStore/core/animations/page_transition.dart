  ...
  PageTransition({
    ...
    this.duration = const Duration(milliseconds: 450),
          ...
          transitionsBuilder: (_, animation, secondaryAnimation, child) {
            final _animatedCurve = CurvedAnimation(parent: animation, curve: curve);
            switch (type) {
              case PageTransitionType.none:
                return child;
              case PageTransitionType.fadeIn:
                return FadeTransition(
                  opacity: Tween<double>(begin: 0, end: 1).animate(_animatedCurve),
                  child: child,
                );
              case PageTransitionType.scaleUp:
                return ScaleTransition(
                  scale: Tween<double>(begin: 0, end: 1).animate(_animatedCurve),
                  child: child,
                );
              case PageTransitionType.scaleDown:
                return ScaleTransition(
                  scale: Tween<double>(begin: 2, end: 1).animate(_animatedCurve),
                  child: child,
                );
              case PageTransitionType.scaleUpWithFadeIn:
                return ScaleTransition(
                  scale: Tween<double>(begin: 0.8, end: 1).animate(_animatedCurve),
              ...
              case PageTransitionType.scaleDownWithFadeIn:
                return ScaleTransition(
                  scale: Tween<double>(begin: 1.2, end: 1).animate(_animatedCurve),
                  child: FadeTransition(
                    opacity: Tween<double>(begin: 0, end: 1).animate(_animatedCurve),
                    child: child,
                  ),
